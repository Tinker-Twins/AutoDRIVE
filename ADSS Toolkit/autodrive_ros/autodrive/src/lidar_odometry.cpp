...
void LIDAR_Odometry::setLaserPose(const Pose3d& laser_pose)
{
...
bool LIDAR_Odometry::is_initialized()
{
  return module_initialized;
}

void LIDAR_Odometry::init(const sensor_msgs::LaserScan& scan,
                          const geometry_msgs::Pose& initial_robot_pose)
{
  // Got an initial laser scan, obtain its parametes
  //ROS_INFO_COND(verbose, "Registering first laser scan... configuring node...");

  width = scan.ranges.size(); // Num of samples (size) of the laser scan
  cols = width; // Max resolution (hould be similar to the width parameter)
  fovh = std::abs(scan.angle_max - scan.angle_min); // LIDAR's horizontal FOV
  ...
  //ROS_INFO_STREAM_COND(verbose, "Setting origin at:\n"
...
const LIDAR_Odometry::Pose3d& LIDAR_Odometry::getIncrement() const
{
  return last_increment_;
}

const Eigen::Matrix<float, 3, 3>& LIDAR_Odometry::getIncrementCovariance() const
{
  return cov_odo;
}

LIDAR_Odometry::Pose3d& LIDAR_Odometry::getPose()
{
  return robot_pose_;
}

const LIDAR_Odometry::Pose3d& LIDAR_Odometry::getPose() const
{
  return robot_pose_;
}

bool LIDAR_Odometry::odometryCalculation(const sensor_msgs::LaserScan& scan)
{
  //============================================================================
  ...
  for (unsigned int i=0; i<ctf_levels; i++)
  {
    ...
    // 2. Calculate inter-scan coordinates
    calculateCoord();
  ...
  //ROS_INFO_COND(verbose, "Execution time (ms): %f",
...
void LIDAR_Odometry::createImagePyramid()
{
  ...
  // The number of levels of pyramid may not match the number of levels used
  // in the odometry computation due to lower resolutions
  ...
  for (unsigned int i = 0; i<pyr_levels; i++)
  {
    ...
    //                   Subsequent levels -> downsampling
    //--------------------------------------------------------------------------
    else
    {
      for (unsigned int u = 0; u < cols_i; u++)
...
void LIDAR_Odometry::calculateCoord()
{
  for (unsigned int u = 0; u < cols_i; u++)
  {
...
void LIDAR_Odometry::calculateRangeDerivativesSurface()
{
  // The maximum gradient size is reserved at the constructor
...
void LIDAR_Odometry::computeNormals()
{
...
void LIDAR_Odometry::findNullPoints()
{
  // The maximum size of null matrix is reserved at the constructor
  num_valid_range = 0;
...
void LIDAR_Odometry::solveSystemOneLevel()
{
  ...
  // The order of variables will be (vx, vy, wz)
...
void LIDAR_Odometry::solveSystemNonLinear()
{
  ...
  // Compute the average dt and average res
  ...
  //printf("\n Avg dt = %f, Avg res = %f", aver_dt, aver_res);
  ...
  //ROS_INFO_STREAM_COND(verbose && false, "Odom covariance:\n" << cov_odo);
}

void LIDAR_Odometry::Reset(const Pose3d& ini_pose)
{
  // Set the initial pose
...
void LIDAR_Odometry::performWarping()
{
...
bool LIDAR_Odometry::filterLevelSolution()
{
  ...
  {
    //ROS_WARN_COND(verbose, "ERROR: Eigensolver couldn't find a solution. Pose is not updated");
    return false;
  }

  // First, describe new linear and angular speeds in the "eigenvector" basis
  //----------------------------------------------------------------------------
  ...
  // Second, describe old linear and angular speeds in the "eigenvector" basis
  //----------------------------------------------------------------------------
  MatrixS31 kai_loc_sub;

  // Important: Substract the solutions from previous levels
  Eigen::Matrix3f acu_trans;
  acu_trans.setIdentity();
  ...
  {
    kai_loc_sub(2) = -fps*std::acos(acu_trans(0,0))*lidar_odometry::sign(acu_trans(1,0));
  }
...
void LIDAR_Odometry::PoseUpdate()
{
  ...
  {
    kai_loc_(2) = fps*std::acos(acu_trans(0,0))*lidar_odometry::sign(acu_trans(1,0));
  }

  //cout << endl << "Arc cos (incr tita): " << kai_loc_(2);

  float phi = lidar_odometry::getYaw(laser_pose_.rotation());
  ...
  pose_aux_2D = lidar_odometry::matrixYaw(double(kai_loc_(2)/fps));
  ...
  //			                    	Compute kai_loc_old
  //----------------------------------------------------------------------------
  phi = lidar_odometry::getYaw(laser_pose_.rotation());
  ...
  //ROS_INFO_COND(verbose, "LIDAR odometry (m): [%f %f %f]",
  //              laser_pose_.translation()(0),
  //              laser_pose_.translation()(1),
  //              lidar_odometry::getYaw(laser_pose_.rotation()));

  // Compose transformations
  robot_pose_ = laser_pose_ * laser_pose_on_robot_inv_;

  //ROS_INFO_COND(verbose, "Robot odometry (m): [%f %f %f]",
  //              robot_pose_.translation()(0),
  //              robot_pose_.translation()(1),
  //              lidar_odometry::getYaw(robot_pose_.rotation()));

  // Estimate linear/angular speeds (mandatory for base_local_planner)
  // last_scan -> Last scan received
  // last_odom_time -> Time of the previous laser scan used to estimate the pose
  //----------------------------------------------------------------------------
  ...
  double ang_inc = lidar_odometry::getYaw(robot_pose_.rotation()) -
                   lidar_odometry::getYaw(robot_oldpose_.rotation());
