...
<p align="justify">
AutoDRIVE Simulator is a digital twin of the AutoDRIVE Testbed, which enables users to virtually prototype their algorithms either due to hardware limitations or as a part of the iterative development cycle.
</p>

<p align="justify">
The AutoDRIVE Simulator is a high-fidelity simulator for autonomous vehicles across scales, configurations, and operational design domains. It is targeted towards simplicity, modularity and flexibility. The simulator is developed atop the Unity game engine, which enables simulation of realistic physics (using a custom dynamics library compatible with NVIDIA’s PhysX Engine) and photorealistic graphics (using custom shaders compatible with Unity’s HDRP). The simulator simulates a comprehensive sensor suite and realistic actuator response, and also features a communication bridge in order to interface with diverse autonomous driving software stacks developed using the AutoDRIVE Devkit. The simulator supports headless and graphical operation, with local as well as distributed computing.
</p>
